...
# BlindDPS
class BlindBlurkernel(torch.nn.Module):
    def __init__(self, kernel_size, kernel, device=None):
        super().__init__()
        self.kernel_size = kernel_size
        self.kernel = kernel
        self.device = device
...
class BlindBlurkernel_RGB(torch.nn.Module):
    def __init__(self, kernel_size, kernel, device=None):
        super().__init__()
        self.kernel_size = kernel_size
        self.kernel = kernel
        ...
        self.seq = torch.nn.Sequential(
            torch.nn.ReflectionPad2d(self.kernel_size//2),
            torch.nn.Conv2d(3, 3, self.kernel_size, stride=1, padding=0, bias=False, groups=1)
        )
